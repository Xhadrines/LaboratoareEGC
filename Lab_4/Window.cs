...
{
    internal class ColoredCube
    {
        private int[,] objVertices;
        private Color[] colorVertices;

        public ColoredCube(int[,] vertices)
        {
            objVertices = vertices;
            InitializeColorVertices();
        }

        private void InitializeColorVertices()
        {
            Random rand = new Random();
            colorVertices = new Color[objVertices.GetLength(1) / 3];
            for (int i = 0; i < colorVertices.Length; i++)
            {
                colorVertices[i] = Color.FromArgb(rand.Next(256), rand.Next(256), rand.Next(256));
            }
        }
    ...
    internal class Window : GameWindow
    {
        ...
        private Color[] colorVertices = {
            Color.White, Color.LawnGreen, Color.WhiteSmoke, Color.Tomato, Color.Turquoise, Color.OldLace, Color.Olive, Color.MidnightBlue, Color.PowderBlue, Color.PeachPuff, Color.LavenderBlush, Color.MediumAquamarine
        };

        private ColoredCube coloredCube;

        private Window() : base(800, 600, new GraphicsMode(32, 24, 0, 8))
        {
            InitializeColorVertices();
        }
